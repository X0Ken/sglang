...
class TestLinearAttnBackends(CustomTestCase):
    ...
    def test_an_unstamped_runner_raises_rather_than_guessing(self):
        """No default for "nobody stamped this".

        `attn_backend_wrapper` stamps before it builds the backends that read the
        stamp, so a missing one means a backend was built outside that path. The
        attribute error names it, which is what the full-attention strings do
        too -- a silent triton fallback would hide the wiring mistake behind a
        working-but-wrong kernel.
        """
        with self.assertRaises(AttributeError):
            _Runner().linear_attn_backends
